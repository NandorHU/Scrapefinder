...
public class FacebookLoginActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        webView.setWebViewClient(new WebViewClient(){
            @Override public void onPageFinished(WebView view, String url) {
                ...
                if(url.contains("facebook.com") && !url.contains("/login")) {
                    ...
                    marketplace.setText("✓ MARKETPLACE KERESÉS INDÍTÁSA");
                    info.setText("Facebook bejelentkezve. Nyomd meg a Marketplace keresés gombot.");
                }
                if(scanning && url.contains("/marketplace/") && url.contains("query=")) {
                    handler.postDelayed(() -> extractMarketplaceResults(view), 3500);
                }
            }
        });
        ...
        if(prefs.getBoolean(KEY_FB_LOGGED_IN,false)) {
            info.setText("Facebook session megtalálva. Indíthatod a Marketplace keresést.");
            marketplace.setText("✓ MARKETPLACE KERESÉS INDÍTÁSA");
    ...
    private void extractMarketplaceResults(WebView view){
        ...
        view.evaluateJavascript(js, value -> {
            try {
                ...
                info.setText("A Marketplace oldal betöltődött, de a találatokat most nem sikerült kiolvasni.");
            }
            scanning=false;
        });
    }
    ...
    private int extractPrice(String s){
        try{
            ...
            if(m.find()){ String d=m.group(1).replaceAll("[^0-9]",""); return d.isEmpty()?0:Integer.parseInt(d); }
